(function() {
  ...
  buildQuery = function() {
    var url;
    return url = "http://api.are.na/v2/channels/random-access-memory/contents?page=" + page + "&per=" + posts_per_page;
  };
  ...
  addImagePost = function(post) {
    ...
    return $("img", imageTemplate).attr("src", post.image.large.url);
  };

  addTextPost = function(post) {
    var textTemplate;
    textTemplate = getTemplate("#textTemplate");
    return $("#content", textTemplate).html(post.content_html);
  };

  addMediaPost = function(post) {
    var mediaTemplate;
    mediaTemplate = getTemplate("#mediaTemplate");
    return $("#video", mediaTemplate).html(post.embed.html);
  };
